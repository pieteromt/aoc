...
def init_units(maze,pwr):  # reading all units (in reading order)
    units = []
    for r in range(len(maze)):
        for c in range(len(maze[0])):
            ...
            if (ch == 'G'):
                ...
                units.append(Unit(ch,r,c,200,pwr))
    return units
...
def do_unit(maze, units, unit):
    ...
    done, wrong = attack(maze, units, targets, unit)
    if done:
        return False, wrong
...
def do_round(maze, units):
    ...
    for unit in units:
        if unit.hp <= 0: # skip dead
            continue
        done, wrong = do_unit(maze, units, unit)
        if done or wrong:
            break
    return done, wrong
...
def process(maze,pwr):
    units = init_units(maze,pwr)
    print("Initially:")
    show(maze, units)
    done = False
    n = 0
    while not done:
        done, wrong = do_round(maze, units)
        if wrong: # elve got killed
            return False
        if done:
            break
        n += 1
