...
def set_ch(rows,cols,data,r,c,val):
    row = data[r]
    data[r] = row[:c] + val + row[c+1:]
...
def set_stuck(rows,cols,data):
    set_ch(rows,cols,data,0,0,'#')
    set_ch(rows,cols,data,0,cols-1,'#')
    set_ch(rows,cols,data,rows-1,0,'#')
    set_ch(rows,cols,data,rows-1,cols-1,'#')
